...
export default function LabManagementPage() {
  ...
  useEffect(() => {
    try {
      ...
      const savedLab = localStorage.getItem('LDC_LAB_CASES');
      if (savedLab) setLabCases(JSON.parse(savedLab));
    } catch (e) { console.error(e); }
  }, []);

  const saveLabCasesToStorage = (updated: LabCase[]) => {
    setLabCases(updated);
    try {
      localStorage.setItem('LDC_LAB_CASES', JSON.stringify(updated));
    } catch (e) { console.error(e); }
  };
